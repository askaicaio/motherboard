// =============================================================
// Automations "AlphaA3", route /automations-alpha-a3
// =============================================================
// ⭐⭐ ONE ROUTE, TWO THEMES. Created 2026-09-24 from a question rather than a
// brief. The user, looking at the AlphaA1/AlphaA2 pair: "right now the light and
// dark pain AlphaA1 and AlphaA2 currently uses different links in order to
// portray the same page in different colors. just from gut feeling, that doesnt
// seem like the normal way devs do night mode. what do you think?"
// **They were right.** This page is the same design done the normal way: one
// URL, one component tree, every colour a token, and a class on the page root
// that swaps the token values. Nothing navigates.
//
// ⚠️⚠️ THE STRUCTURE IS ALPHAA1'S, UNCHANGED, and AlphaA1's was the live
// hub's. **ONLY THE COLOUR LAYER DIFFERS.** If you are changing a class that is
// not a colour, a ring width or a divider, you are editing the wrong page. The
// three structural additions are named below and each exists so that the two
// themes can share one box.
//
// 🎨 THE PALETTES AND THE SWAP LIVE IN `theme.tsx`. Read that file first; it is
// short, and it is the actual subject of this experiment. The one-line version:
// **tokens are named for the JOB they do, never for the colour they are**,
// because A1's colour-named tokens could not be reused (its single amber
// `--pa-label` is SEVEN different colours on the light page).
//
// 🔧 THE THREE STRUCTURAL ADDITIONS, none of which change geometry:
//   1. **RING WIDTH IS A TOKEN.** `ring-[length:var(--a-ring-w)]` is 3px in
//      dark and 1px in light; `--a-ring-outer-w` is 0px in dark and 1px in
//      light. ⭐ **This is free because a ring paints OUTSIDE the border box and
//      costs no layout**, which is the single fact that lets one tree wear both
//      frames.
//   2. **DIVIDERS KEEP THEIR BOX AND CHANGE COLOUR.** The toolbar's `border-l`
//      and the rail's `border-r` are present in BOTH themes and painted
//      `transparent` in dark, where the 3px chrome gutter is already the
//      divider. Dropping them in dark instead would move the content by a pixel
//      and the two themes would stop being the same page.
//   3. **THE PANEL HEADER SHELF IS `pb-[2px]` PLUS A 1px BORDER IN BOTH.** In
//      dark both are chrome-coloured, so they read as AlphaA1's solid 3px band;
//      in light the border is the app's hairline over a 2px white shelf, which
//      is AlphaA2. **3px of height either way**, which is what keeps this page
//      aligned with both of its ancestors.
//
// 📌 WHAT GOT DELETED RATHER THAN PORTED, and it is the honest measure of the
// change: the `?site=` prop the toggle needed in order to carry the selected
// website across a navigation, and the `HoverPrefetchLink` warm-up that existed
// to hide the cost of that navigation. **One route means there is no trip for
// the selection to survive.** (`HoverPrefetchLink` is still used by the rail
// cards, which really do navigate.)
//
// ⚠️ IT IS A BENCH, NOT A REPLACEMENT. AlphaA1 and AlphaA2 are PARKED awaiting
// a business decision and this does not change that; see `versions.ts`. **Do not
// delete the pair**: they are what this page is evidence against, and the
// comparison is the point.
// 🛑 THE DARK THEME HERE IS MEANT TO BE PIXEL-IDENTICAL TO ALPHAA1 AND THE LIGHT
// THEME TO ALPHAA2. That is the claim this page makes, so **if you change a
// colour here, either change it on the matching ancestor too or write down why
// they now differ.** There is one known, deliberate difference, recorded in
// `theme.tsx`: because this root carries the `dark` class, shadcn's own dark
// tokens apply inside it, so popovers and tooltips are dark here where AlphaA1
// leaves them light.
//
// ⚠️ SELF-CONTAINED, like every version page. **Do not extract anything shared
// between this file and `automations/page.tsx` or the AlphaA pair**, however
// identical the structure looks. That identity is the point of the experiment,
// and keeping the files apart is what stops a bench breaking the live hub.
//
// ⬇️⬇️ EVERYTHING BELOW IS THE LIVE HUB'S OWN HEADER, INHERITED WITH THE COPY.
// It describes /automations, its history and its layout reasoning, all of which
// still applies because the structure is identical. Read "this page" as "the
// live hub", and read any colour name in it as the LIVE page's colour.
// =============================================================

// =============================================================
// (INHERITED) Automations Main Page, the LIVE hub at route /automations.
// =============================================================
...
import {
  AlertTriangle,
  // Housekeeping's toolbar icon, ported with that cell 2026-09-24. A broom
  // reads as tidying-up work, which is what the page it leads to is.
  Brush,
  Inbox,
  List,
  ListChecks,
...
/* 🎨🎨 THE COLOUR VALUES ARE NOT IN THIS FILE ANY MORE. Both palettes live in
 * `theme.tsx`, because there are two of them now and a page that renders one
 * tree in two themes should not look like it owns either.
 *
 * ⚠️⚠️ THE PROSE BELOW IS ALPHAA1'S, KEPT VERBATIM, AND IT NAMES TOKENS
 * `--pa-*`. **Those names do not exist on this page.** They were renamed because
 * they could not survive a second theme: they are named after COLOURS, and the
 * light palette needs different colours in the same slots. The rough mapping,
 * for reading the notes below:
 *     --pa-void   -> --a-well / --a-page / --a-tile   (three roles, one colour)
 *     --pa-card   -> --a-lift          --pa-inset -> --a-sel / --a-lift-strong
 *     --pa-line   -> --a-chrome / --a-ring / --a-track-off / --a-icon-faint
 *     --pa-label  -> the six --a-text-* roles, plus --a-warn-*
 *     --pa-blue   -> --a-value / --a-value-faint / --a-ok-text
 *     --pa-red    -> --a-problem / --a-problem-note / --a-bad-* / --a-off-text
 * **The one-to-many rows are the whole reason this page exists.** A dark theme
 * that collapses six greys into one amber reads beautifully and cannot be
 * un-collapsed later by renaming variables; the roles have to be recovered from
 * the light design, which is what `theme.tsx` does.
 *
 * 📌 IT IS KEPT VERBATIM RATHER THAN REWRITTEN because it is the user's own
 * reasoning about the palette, argued out over several days, and a paraphrase
 * would lose the parts that were corrections. Everything it says about the DARK
 * theme is still exactly true. */
...
const TOOL_SEGMENT =
  ...
  "flex items-center justify-center gap-2 border-[color:var(--a-divider)] bg-[var(--a-well)] px-4 py-3 text-sm font-medium text-[var(--a-text-body)] transition-colors hover:bg-[var(--a-lift-strong)] hover:text-[var(--a-text-strong)]";
...
const COVERAGE_FIELDS: {
  ...
  // ⚠️ PORTED FROM THE LIVE HUB 2026-09-24 ("Update AlphaA1 and AlphaA2 to have
  // the recent updates to the main page").
  // 🛑 THE SET IS THE FOUR *REQUIRED* FIELDS NOW, and the order is the
  // Housekeeping table's left-to-right column order, not a ranking.
  // **Author and Notes LEFT, Evaluation ARRIVED**: the live panel was renamed
  // "Documentation of Required Fields" (#574) and the required set dropped to
  // four when Notes was demoted to supporting information (#563). Author was
  // never required - it is empty on 916 of 917 rows, so it would flag the whole
  // estate. ⚠️ Re-adding either means re-adding its count to `coverageBase`.
...
export default async function AutomationsAlphaA3Page({
  searchParams,
}: {
  ...
  // ⭐⭐ THE THEME IS DECIDED HERE, ON THE SERVER, and that is the whole reason
  // this page needs no flash-prevention script on any path in. A cookie rides
  // along with a full document request AND with the RSC request for a
  // client-side navigation, so the attribute below is already right in the
  // first HTML either way.
  // ⭐ NO COOKIE MEANS "FOLLOW THE OS", which the server cannot see - so it
  // renders NO attribute at all and the CSS media query decides, before paint
  // and without JavaScript. **The absent attribute is the feature.** See
  // `A3_THEME_ATTR` for the bug that got us here.
  const cookieTheme = (await cookies()).get(A3_THEME_COOKIE)?.value;
  const theme =
    ...
    await Promise.all([
      ...
      db
        .select({
          ...
          // ⚠️ PORTED FROM THE LIVE HUB 2026-09-24 ("Update AlphaA1 and AlphaA2 to have
          // the recent updates to the main page"). `triage` (Evaluation) replaced `notes` and
          // `author` here when `COVERAGE_FIELDS` became the required four.
      ...
      // How many automations the Housekeeping page would list, for the count
      // pill in the toolbar strip. ONE aggregate over the same predicate that
      // page uses, so the pill and the page cannot disagree.
      // ⚠️ PORTED FROM THE LIVE HUB 2026-09-24, and it is the ONLY new query
      // this port added. It rides in the wave that was already here rather than
      // opening a third: this page runs two waves against a pool of 10
      // ([[db-pool-max-10-fanout]]), and the first wave now has five reads.
      // ⚠️ WHOLE ESTATE, NOT `selected.slug`. The page it links to opens on all
      // websites, so scoping it to the selected site would have the toolbar
      // promising one number and the page showing another.
  ...
  // ⚠️ `triage` (shown as Evaluation) IS A ROW-LEVEL COLUMN and comes from
  // query 1, not the junction: it is a single-select `triageChoiceId`. This
  // note used to say it came back from query 2, which was wrong and harmless
  // only because the field was not in `COVERAGE_FIELDS` at the time.
  ...
  // 🛑🛑 NO SORT, PORTED FROM THE LIVE HUB 2026-09-24. The rows render in
  // `COVERAGE_FIELDS` order, which is the Housekeeping table's left-to-right
  // column order.
  // ⚠️ THIS USED TO SAY THE OPPOSITE, and said it emphatically: "SORTED THINNEST
  // FIRST ... the order IS the recommendation about what to fill in next. Do not
  // re-sort into table order." **That was Alpha6's editorial idea and the user
  // replaced it on 2026-09-17** ("in that order. from left to right, instead
  // being top to bottom"). The panel no longer recommends what to fill next; it
  // reports the same fields in the same order everywhere they appear, which is
  // also why the "thinnest first" hint went. **Do not restore the sort on the
  ...
  /** The most recent day in the window that has errors, and how many.
   *
   *  ⚠️ PORTED FROM THE LIVE HUB 2026-09-24. Replaces the old "Last Error N
   *  days ago" line, which said WHEN but never HOW MANY.
   *  ⭐ DERIVED FROM `trend`, THE CHART'S OWN DATA, AND NOT A NEW QUERY. This
   *  page already runs two waves against a pool of 10
   ...
   *  keys the bars use, while the older wording used `days`, which is ELAPSED
   *  time floored to whole days. The two can differ by one around midnight, and
   *  **calendar days are the right unit next to a daily chart**. */
  const lastErrorDay = (() => {
  ...
  return (
    ...
    <div
      id={A3_ROOT_ID}
      // ⭐ NO `suppressHydrationWarning` ANYWHERE ON THIS PAGE, and that is a
      // result rather than an omission: the server and the client render the
      // same attribute every time, because the only thing that can disagree
      // with them - the OS preference - is handled in CSS and never touches
      // the DOM. The class-plus-script version needed it and still lost.
      {...(theme ? { [A3_THEME_ATTR]: theme } : {})}
      // ⚠️ THE PAGE'S INHERITED TEXT COLOUR IS A ROLE OF ITS OWN,
      // `--a-text-base`: amber in dark, as AlphaA1 set it on 2026-09-12, and
      // the app's own `--foreground` in light. Anything without a colour class
      // of its own lands here, and the h1 is the one that matters: it has no
      // class, so in dark it would otherwise inherit the LIGHT dashboard's
      // near-black.
      // ⚠️ `space-y-5` AND `p-12` MOVED OFF THIS DIV on 2026-09-24, onto the
      // floored div below. **The full-bleed escape stays here**: `-m-6`,
      // `min-h-screen` and the background are what make the dark surface cover
      // `main`'s padding, and they must stay OUTSIDE the scroller so the dark
      // ground still fills the viewport while the content scrolls under it.
      // 📌 NO `[color-scheme:dark]` HERE ANY MORE. It is per-theme now and
      // lives in `theme.tsx`, which sets it for light as well, so native
      // scrollbars and form controls follow the page either way.
      className="-m-6 min-h-screen bg-[var(--a-page)] text-[var(--a-text-base)]"
    >
      {/* ⚠️ BOTH PALETTES, INLINE AND SCOPED TO THIS PAGE. No script sits beside
          it any more; see `A3_THEME_ATTR` for why there used to be one. */}
      <ThemeStyles />
      {/* ⭐⭐ THE PAGE KEEPS ITS WIDTH AND SCROLLS SIDEWAYS, ported from the live
       ...
       is `min-h-screen`, and `overflow-x: auto` makes the OTHER axis compute to
       auto too. Put it on a full-height element and the page gets its own
       VERTICAL scroll container, which stops the window scrolling the page and
       re-anchors any `position: sticky` inside it. The wrapper has auto height,
       so its vertical overflow never engages.
       🛑 AND NOT ON `<main>` EITHER: that is `overflow-x-clip`, which cuts
       overflow off WITHOUT creating a scroll container, and it is shared by
       every dashboard page.
       📊 WHY THE FLOOR IS 929px OF CONTENT, measured on both halves, which come
       out identical once their different padding is accounted for. Three things
       break below it and the TOOLBAR is the one this port introduced:
         1. THE TOOLBAR STRIP. The Housekeeping cell made it `grid-cols-4`, and
            at 869 of content "Dropdown Configuration" wraps to two lines and the
            strip grows 50px -> 70px. **Four cells need more width than three
            did; that is the cost of the new cell.**
         2. THE COVERAGE BAR, the only flexible thing in its row: 132px at 929 of
            content, 102 at 899, 72 at 869, **2 at 799**.
         3. THE SPARKLINE'S PEAK CAPTION, which runs into "30 days ago": 26px of
            clearance at 929, 11 at 899, **-4 at 869**.
       ⭐ 929 IS THE APPEARANCE AT A 1280px WINDOW, which is the narrowest the
       page looks right at. Below it the page scrolls sideways instead.
       ⚠️ THE NUMBER DIFFERS FROM THE LIGHT TWIN'S AND THE GEOMETRY DOES NOT:
       this half pads with `p-12` and A2 with `p-6`, so 929 of content is
       `min-w-[1025px]` here and `min-w-[977px]` there. **Measured on both; they
       agreed to the pixel.** */}
      <div className="overflow-x-auto">
        <div className="min-w-[1025px] space-y-5 p-12">
          ...
          <TooltipProvider delay={TOOLTIP_DELAY_MS}>
            <HealthCheckProvider>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <Workflow className="h-5 w-5 text-[var(--a-icon)]" />
                    ...
                    <span className="rounded-full bg-[var(--a-badge-bg)] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--a-badge-text)] ring-1 ring-[color:var(--a-badge-ring)]">
                      AlphaA3
                    </span>
                  ...
                  <p className="mt-1 text-sm text-[var(--a-text-muted)]">
                    Tracks workflows from different automation websites all in
                    one place.
                  </p>
                </div>
                ...
                <div className="flex shrink-0 items-center gap-3">
                  ...
                  <AutoHealthCheckToggle
                    ...
                    classNames={{
                      label: "text-[var(--a-text-dim)]",
                      countdown: "text-[var(--a-text-muted)]",
                      error: "text-[var(--a-problem)]",
                    }}
                  />
                  ...
                  <ApiHealthCheckButton className="bg-[var(--a-hbtn-bg)] text-[var(--a-hbtn-text)] ring-1 ring-[color:var(--a-hbtn-ring)] hover:bg-[var(--a-hbtn-hover)]" />
                </div>
              </div>
              ...
              **do not re-add the rail section as well** and leave the estate
              ...
              {/* ⚠️ PORTED FROM THE LIVE HUB 2026-09-24 ("Update AlphaA1 and AlphaA2 to have
              the recent updates to the main page"). A FOURTH CELL, Housekeeping,
              so `grid-cols-3` became `grid-cols-4` and the right-hand radius
              moved off Dropdown Configuration onto the new last cell. */}
              <div className="grid grid-cols-4 gap-[3px] overflow-hidden rounded-xl bg-[var(--a-chrome)] p-[3px] ring-[length:var(--a-ring-outer-w)] ring-[color:var(--a-ring)]">
                <Link
                  href="/automations/feature-integration"
                  className={cn(TOOL_SEGMENT, "rounded-l-[11px]")}
                >
                  <Plug className="h-4 w-4 text-[var(--a-icon)]" />
                  Feature Integration
                </Link>
                <Link
                ...
                >
                  <List className="h-4 w-4 text-[var(--a-icon)]" />
                  View All Lists
                </Link>
                <Link
                  ...
                  className={cn(TOOL_SEGMENT, "border-l")}
                >
                  <ListChecks className="h-4 w-4 text-[var(--a-icon)]" />
                ...
                >
                  <Brush className="h-4 w-4 text-[var(--a-icon)]" />
                  Housekeeping
                  {/* ⚠️ THE LABEL LOST "Alerts" AND THE ROUTE DID NOT: it is still
                  `/automations/housekeeping-alerts`. Do not rename it to match
                  without a redirect.
                  🎨🎨 THE PILL IS **RED ON A DARK WELL HERE, NOT LIVE'S AMBER**,
                  and that is a deliberate choice the user made on 2026-09-24
                  rather than a porting slip. Live uses `bg-amber-100` /
                  `text-amber-800` because amber is the Housekeeping feature's
                  colour end to end over there. **Neither half of that survives
                  this page's rules**: amber here means A LABEL (see the
                  three-colour note), and `amber-100` is a light surface behind
                  text, which the window hierarchy forbids outright. Red means A
                  PROBLEM, which is what a backlog count is, and `--pa-inset` is
                  one of the four dark tiers text is allowed to sit on.
                  ...
                  {housekeeping > 0 ? (
                    <span className="rounded-full border border-[color:var(--a-pill-line)] bg-[var(--a-pill-bg)] px-2 py-0.5 text-xs font-semibold tabular-nums text-[var(--a-pill-text)]">
                      {housekeeping}
                    </span>
                  ) : null}
                </Link>
              ...
              <div className="group/pane flex min-h-[640px] gap-[3px] overflow-hidden rounded-xl bg-[var(--a-chrome)] p-[3px] ring-[length:var(--a-ring-outer-w)] ring-[color:var(--a-ring)]">
                ...
                <div className="flex w-[460px] shrink-0 flex-col rounded-l-[11px] border-r border-[color:var(--a-divider)] bg-[var(--a-well)]">
                  ...
                  **So if a portfolio total is ever wanted back, it is a NEW
                  ...
                  <nav className="flex-1 space-y-1.5 p-2">
                    {AUTOMATION_SITES.map((site) => {
                      ...
                      const sActivePct = s.total
                        ? (s.active / s.total) * 100
                        : 0;
                      const sPausedPct = s.total
                        ? (s.paused / s.total) * 100
                        : 0;
                      return (
                        ...
                        <div
                          ...
                          className={cn(
                            ...
                            isCurrent
                              ? "bg-[var(--a-sel)]"
                              : "bg-[var(--a-well)] hover:bg-[var(--a-lift)]",
                          )}
                        >
                          ...
                          **an <a> may not contain any of them**. Wrapping the
                          ...
                          <HoverPrefetchLink
                            href={`/automations-alpha-a3?site=${site.slug}`}
                              ...
                              **Do not repeat my assumption: measure it if the
                              ...
                              <span className="flex min-w-0 flex-1 items-center gap-2.5">
                                  ...
                                  **h-8 + text-base is the largest that is FREE:
                                  ...
                                  <span
                                    className={cn(
                                      ...
                                      // ⚠️⚠️ THE TWO STATES CARRY A COLOUR ROLE EACH
                                      // EVEN THOUGH DARK PAINTS THEM THE SAME. AlphaA1
                                      // made both amber on 2026-09-12 (a website's name
                                      // is a LABEL) and lets the WEIGHT and the card's
                                      // lift carry selection. **Light needs the two to
                                      // differ** (zinc-900 against zinc-700), so the
                                      // distinction has to exist in the markup and be
                                      // collapsed by the palette, not the other way
                                      // round. This is the one-to-many problem in
                                      // miniature; see the pointer note near the top.
                                      isCurrent
                                        ? "font-semibold text-[var(--a-text-strong)]"
                                        : "font-medium text-[var(--a-text-body)]",
                                    )}
                                  >
                                    {site.label}
                                  </span>
                                ...
                                <span className="flex shrink-0 flex-col items-start gap-0.5">
                                  ...
                                  <span
                                    ...
                                    className="flex items-center gap-1 whitespace-nowrap text-[11px] text-[var(--a-text-muted)]"
                                  >
                                    ...
                                    **`RefreshCwOff` is the same glyph with a
                                    ...
                                    {siteRefreshOn ? (
                                      <RefreshCw className="h-3 w-3 shrink-0 text-[var(--a-refresh-on)]" />
                                    ) : (
                                      <RefreshCwOff className="h-3 w-3 shrink-0 text-[var(--a-icon)]" />
                                    )}
                                    {siteRefreshOn
                                      ? "Auto-refresh on"
                                      : "Auto-refresh off"}
                              ...
                              <span className="relative z-10 flex shrink-0 items-center gap-2">
                                ...
                                <Link
                                  ...
                                  className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-[var(--a-chip-bg)] px-2.5 text-xs font-medium text-[var(--a-chip-text)] ring-1 ring-[color:var(--a-ring)] transition-colors hover:bg-[var(--a-chip-hover-bg)] hover:text-[var(--a-chip-hover-text)]"
                                >
                            ...
                            <span className="block">
                              <span className="flex items-baseline justify-between gap-2">
                                <span className="flex min-w-0 items-baseline gap-1">
                                  ...
                                  <span className="font-heading text-lg font-semibold leading-none tabular-nums text-[var(--a-value-hero)]">
                                    {s.total}
                                  </span>
                                  <span className="truncate text-[10px] text-[var(--a-text-muted)]">
                                    automations
                                  </span>
                                </span>
                                <span className="flex shrink-0 items-center gap-2 text-[10px] text-[var(--a-text-dim)]">
                                  <span className="flex items-center gap-1">
                                    ...
                                    />
                                    <span className="font-semibold tabular-nums text-[var(--a-value)]">
                                      {s.active}
                                    </span>
                                    active
                                  </span>
                                  <span className="flex items-center gap-1">
                                    <span className="h-1.5 w-1.5 rounded-full bg-[var(--a-track-off)]" />
                                    <span className="font-semibold tabular-nums text-[var(--a-value)]">
                                      {s.paused}
                                    </span>
                                    paused
                              ...
                              <span className="mt-1.5 flex h-1 w-full overflow-hidden rounded-full bg-[var(--a-track)]">
                                <span
                                  style={{
                                    width: `${sActivePct}%`,
                                    backgroundColor: ACCENT[site.slug],
                ...
                <div className="@container min-w-0 flex-1 rounded-r-[11px] bg-[var(--a-well)] transition-opacity duration-200 group-has-[[data-pending]]/pane:opacity-60">
                      ...
                      <div className="flex min-w-0 flex-col gap-4">
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex min-w-0 items-center gap-3">
                            <span
                              aria-hidden
                              className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[var(--a-tile)] ring-1 ring-[color:var(--a-ring)]"
                            >
                            ...
                            <div className="min-w-0">
                              <h2 className="font-heading text-xl font-semibold text-[var(--a-text-strong)]">
                                {selected.label}
                              </h2>
                              <p className="mt-0.5 text-sm text-[var(--a-text-dim)]">
                          ...
                          **#486 PUT THIS HEADER IN THE LEFT COLUMN OF A TWO-COLUMN
                          ...
                          <Link
                            ...
                            className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg bg-[var(--a-chip-bg)] px-2.5 text-xs font-medium text-[var(--a-chip-text)] ring-1 ring-[color:var(--a-ring)] transition-colors hover:bg-[var(--a-chip-hover-bg)] hover:text-[var(--a-chip-hover-text)]"
                          >
                        ...
                        **98px is still the right number for the cards question**,
                        ...
                        <div className="flex flex-1 flex-col rounded-lg bg-[var(--a-well)] p-3 ring-[length:var(--a-ring-w)] ring-[color:var(--a-ring)]">
                          <div className="flex items-center justify-between gap-3">
                            ...
                            {/* ⚠️ PORTED FROM THE LIVE HUB 2026-09-24: "Instead of
                            it mentioning when the last error happened, do
                            something like 'X errors Y days ago'. The X being a
                            red number."
                            ⚠️ THE COUNT IS A VALUE BUT IT IS RENDERED RED HERE,
                            not blue, and that is this page's rule rather than a
                            slip: red means A PROBLEM, and error counts are the
                            named example. The figure beside it at the top of
                            this block is red for the same reason.
                            ⚠️ THE FALLBACK IS THE OLD SENTENCE, for a website
                            whose last error is OLDER than the chart's window:
                            the per-day data cannot supply a count there, and
                            inventing one from `days` would mean two different
                            definitions of a day in one line. "not tracked yet"
                            still covers a website that has never captured one
                            (permanent for GHL, GHL b2b and Zapier). */}
                            <span className="text-[11px] text-[var(--a-text-muted)]">
                              {days === undefined ? (
                                "not tracked yet"
                              ) : lastErrorDay ? (
                                <>
                                  <span className="font-semibold tabular-nums text-[var(--a-problem)]">
                                    {lastErrorDay.count}
                                  </span>{" "}
                                  {lastErrorDay.count === 1
                                    ? "error"
                                    : "errors"}{" "}
                  ...
                  <div className="p-6">
                    ...
                    layout gives one website the whole canvas. */}
                      <Panel
                        // ⚠️ PORTED FROM THE LIVE HUB 2026-09-24 ("Update AlphaA1 and AlphaA2 to have
                        // the recent updates to the main page"). The hint said "on the
                        // website", which the title already says; it now carries the
                        // ROW COUNT, which nothing else on the panel did.
                        ...
                        {recentlyEdited.map((row) => (
                          <li key={row.id}>
                            <Link
                              ...
                              className="block px-3.5 py-2.5 transition-colors hover:bg-[var(--a-lift)]"
                            >
                              <div className="flex items-baseline justify-between gap-2">
                                <span className="truncate text-sm font-medium text-[var(--a-value)]">
                                  {row.name}
                                </span>
                                <span className="shrink-0 text-[11px] tabular-nums text-[var(--a-value-faint)]">
                                  {agoLabel(
                                    row.lastEditedAt
                              ...
                              <span className="mt-1 inline-flex items-center gap-1 text-[11px] text-[var(--a-text-muted)]">
                        ...
                        {siteErrors.map((row) => (
                          <li key={row.id}>
                            ...
                            >
                              ...
                              <p className="mt-0.5 truncate text-xs text-[var(--a-problem-note)]">
...
const TONE_CLASSES: Record<Tone, string> = {
  ok: "text-[var(--a-ok-text)]",
  warn: "text-[var(--a-warn-text)]",
  bad: "text-[var(--a-bad-text)]",
  off: "text-[var(--a-off-text)]",
};

const TONE_DOTS: Record<Tone, string> = {
  ok: "bg-[var(--a-ok-dot)]",
  warn: "bg-[var(--a-warn-dot)]",
  bad: "bg-[var(--a-bad-dot)]",
  off: "bg-[var(--a-off-dot)]",
};
...
function Sparkline({
  ...
  return (
    <div className="mt-2.5 flex flex-1 flex-col">
      <div className="flex min-h-7 flex-1 items-end gap-[3px]">
        {values.map((v, i) => (
          <span
            ...
            className={cn(
              "flex-1 rounded-[2px]",
              v > 0 ? "bg-[var(--a-err-bar)]" : "bg-[var(--a-err-bar-zero)]",
            )}
            style={{
      ...
      {/* ⭐⭐ THE FOOTER IS AN AXIS, PORTED FROM THE LIVE HUB 2026-09-24: ends
          marking where the window starts and finishes, peak in the middle.
          **The chart finally says WHEN as well as how much** - before this the
          bars were 30 unlabelled columns and the caption only gave the window
          length.
          ⭐ THE PEAK GIVES THE BARS THE SCALE THEY NEVER HAD. Every chart is
          ...
          ⚠️ AT ZERO IT SAYS SO IN WORDS rather than falling back to a bare
          window caption: an empty chart with only an axis looks like something
          failed to load, while "no errors" is a RESULT. It covers both zeros, a
          tracked website with a quiet month and one that captures nothing at
          all; the line above already separates them.
          ⚠️ `grid-cols-3` AND NOT `justify-between`: equal thirds keep the
          middle label CENTRED UNDER THE CHART whatever the end labels weigh.
          🎨 The peak count is red because it is an error count; the words
          around it are labels, so they are amber. */}
      <div className="mt-1.5 grid grid-cols-3 items-baseline text-[10px] uppercase tracking-wider text-[var(--a-text-faint)]">
...
function Panel({
  ...
  return (
    <div className="overflow-hidden rounded-lg bg-[var(--a-well)] ring-[length:var(--a-ring-w)] ring-[color:var(--a-ring)]">
      ...
      <div className="bg-[var(--a-chrome)] pb-[2px]">
        <div className="flex items-center justify-between gap-2 border-b border-[color:var(--a-header-line)] bg-[var(--a-header)] px-3 py-1.5">
          <span className="text-xs font-semibold text-[var(--a-text-head)]">
            {title}
          </span>
          <span className="text-[10px] uppercase tracking-wider text-[var(--a-text-muted)]">
            {hint}
          </span>
        </div>
      </div>
      {empty ? (
        <div className="flex flex-col items-center justify-center gap-2 px-4 py-8 text-center">
          <Inbox className="h-5 w-5 text-[var(--a-icon-faint)]" />
          <p className="text-xs text-[var(--a-text-muted)]">{emptyLabel}</p>
        </div>
      ) : (
...
function CoverageByField({
  ...
  return (
    <div className="min-w-0 overflow-hidden rounded-lg bg-[var(--a-well)] ring-[length:var(--a-ring-w)] ring-[color:var(--a-ring)]">
      ...
      <div className="bg-[var(--a-chrome)] pb-[2px]">
        <div className="flex items-center justify-between gap-2 border-b border-[color:var(--a-header-line)] bg-[var(--a-header)] px-3 py-1.5">
          {/* ⚠️ PORTED FROM THE LIVE HUB 2026-09-24 ("Update AlphaA1 and AlphaA2 to have
              the recent updates to the main page"). The hint went with the rename:
              it read "thinnest first" until the sort was dropped, then "required
              columns", which the new title says in full. **A hint that repeats
              its title is noise.** */}
          <span className="text-xs font-semibold text-[var(--a-text-head)]">
        ...
        <ul className="divide-y">
          {rows.map((row) => (
            <li key={row.key} className="flex items-center gap-3 px-3.5 py-2">
              <span className="w-28 shrink-0 truncate text-xs font-medium text-[var(--a-text-body)]">
                {row.label}
              </span>
              <div className="flex h-1.5 min-w-0 flex-1 overflow-hidden rounded-full bg-[var(--a-track)]">
                <span
                  className={cn("rounded-full", barClass(row.pct))}
                  style={{ width: `${Math.min(100, row.pct)}%` }}
                />
              </div>
              <span className="w-10 shrink-0 text-right text-xs font-semibold tabular-nums text-[var(--a-value)]">
                {Math.round(row.pct)}%
              </span>
              <span className="w-16 shrink-0 text-right text-[11px] tabular-nums text-[var(--a-value-faint)]">
                {row.filled}/{total}
              </span>
            </li>
          ))}
...
function barClass(p: number): string {
  // ⚠️ PORTED FROM THE LIVE HUB 2026-09-24 ("Update AlphaA1 and AlphaA2 to have
  // the recent updates to the main page"). Live's three steps at 30/70, replacing
  // four at 20/45/70 whose top two both returned green anyway, so this is a
  // threshold change rather than a step-count change.
  if (p < 30) return "bg-[var(--a-bar-low)]";
  if (p < 70) return "bg-[var(--a-bar-mid)]";
  return "bg-[var(--a-bar-high)]";
}
