// =============================================================
// Automations "AlphaA1", route /automations-alpha-a1
// =============================================================
// ⭐ A RECOLOUR BENCH, THE FIRST OF ITS KIND HERE. Created 2026-09-11: "AlphaA1
// has the previous color list as its choices for color scheme. AlphaA1 is an
// experimental page that uses the same UI structure of the current live page,
// but the colors all come from the color list earlier."
//
// ⚠️⚠️ THE STRUCTURE IS THE LIVE HUB'S, UNCHANGED. Every layout decision, query
// and component is a copy of `automations/page.tsx` as it stood on 2026-09-11.
// **ONLY COLOUR DIFFERS.** If you are changing a class that is not a colour,
// you are editing the wrong page.
//
// 🎨 THE PALETTE IS NINE COLOURS, read by eye off a screenshot of a pixel-art
// mech-factory game UI the user shared, then curated by them from seventeen down
// to nine. It is written down ONCE, in `PALETTE` below, and reaches the markup
// as CSS custom properties. **To retune this page, edit that object and nothing
// else.**
//
// ⭐⭐ HOW THE RECOLOUR REACHES CODE THAT NEVER MENTIONS COLOUR, which is what
// made this feasible at all: `globals.css` declares
// `@theme inline { --color-border: var(--border); ... }`, so Tailwind's token
// utilities resolve their variable AT USE TIME instead of baking a value in.
// **Overriding `--background`, `--foreground`, `--card`, `--muted` and
// `--border` on this page's root therefore recolours every `bg-card`,
// `bg-muted/40`, `border`, `border-b`, `divide-y` and `ring-foreground/10`
// inside it, with no class edits at all.** That covered roughly 40 of the ~100
// colour sites. Only the explicit `zinc-*` / `emerald-*` / `amber-*` / `red-*`
// classes needed swapping, and they now read `[var(--pa-*)]`.
//
// ⚠️ THIS IS A LIGHT-TO-DARK INVERSION, NOT A HUE SWAP. The live page is a light
// theme and this palette is a dark one, so "darkest text" became "brightest
// text" and "lightest fill" became "darkest fill". **Expect some things to read
...
// between this file and `automations/page.tsx`**, however identical the
// structure looks. That identity is the point of the experiment, and keeping the
// files apart is what stops a bench breaking the live hub.
//
...
// live hub", and read any colour name in it as the LIVE page's colour, not
// this one's.
// =============================================================
...
/** The nine colours, and the ONLY place any of them is written down.
 *
 *  Names describe the JOB, not the hue, so a retune does not turn every name
 *  into a lie. The numbers are the user's own from the curated list. */
const PALETTE = {
  /** 1. Page ground, the darkest thing on screen. */
  void: "#0A0E13",
  /** 5. One step off the void: pane and card surfaces. */
  card: "#16202A",
  /** 2. Inset fills: bar tracks, header strips, hover states. */
  inset: "#2A3038",
  /** 3. Hairlines, dividers, rings. */
  line: "#4A5560",
  /** 4. Muted text and secondary icons. */
  muted: "#8A96A2",
  /** 6. The game's own label colour, and this page's primary text. */
  label: "#E8A33C",
  /** 8. Brightest text, for the few things that must out-rank the labels. */
  bright: "#DCE2E8",
  /** 7. Errors and error counts. */
  red: "#C0392F",
 ...
 *  `bg-card`, `bg-muted`, `border`, `divide-y` and `ring-foreground/10` on the
 *  page without touching a class.
 *  ⚠️ `--foreground` MUST BE THE BRIGHT COLOUR, not the label colour:
 *  `ring-foreground/10` reads it, and those hairlines are light-on-dark here
 *  where they were dark-on-light before. Set it to amber and every ring turns
 *  into a faint orange line. */
const PAGE_VARS = {
  ...
  "--pa-card": PALETTE.card,
  "--pa-inset": PALETTE.inset,
  "--pa-line": PALETTE.line,
  "--pa-muted": PALETTE.muted,
  "--pa-label": PALETTE.label,
...
export default async function AutomationsAlphaA1Page({
  searchParams,
}: {
  ...
  return (
    ...
    <div
      ...
      className="-m-6 min-h-screen bg-[var(--pa-void)] text-[var(--pa-label)] [color-scheme:dark]"
    >
          ...
          <TooltipProvider delay={TOOLTIP_DELAY_MS}>
            <HealthCheckProvider>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <Workflow className="h-5 w-5 text-[var(--pa-muted)]" />
              ...
              **do not re-add the rail section as well** and leave the estate
              ...
              <div className="grid grid-cols-4 gap-[3px] overflow-hidden rounded-xl bg-[var(--pa-line)] p-[3px]">
                <Link
                ...
                >
                  <Brush className="h-4 w-4 text-[var(--pa-muted)]" />
                  Housekeeping
                      ...
                      return (
                        ...
                        >
                          ...
                          **an <a> may not contain any of them**. Wrapping the
                          ...
                          <HoverPrefetchLink
                            href={`/automations-alpha-a1?site=${site.slug}`}
                          ...
                          **So the pair now sits in the TITLE ROW only, and the
                          ...
                          <div className="flex min-w-0 flex-1 flex-col gap-1.5">
                                ...
                                <span className="flex shrink-0 flex-col items-start gap-0.5">
                                  ...
                                  >
                                    ...
                                    **`RefreshCwOff` is the same glyph with a
                                    ...
                                    {siteRefreshOn ? (
                                      <RefreshCw className="h-3 w-3 shrink-0 text-[var(--pa-green)]" />
                                    ) : (
                                      <RefreshCwOff className="h-3 w-3 shrink-0 text-[var(--pa-muted)]" />
                                    )}
                                    {siteRefreshOn
                                      ? "Auto-refresh on"
                                      : "Auto-refresh off"}
                            ...
                            <span className="block">
                              ...
                              <span className="mt-1.5 flex h-1 w-full overflow-hidden rounded-full bg-[var(--pa-inset)]">
                                <span
                                  style={{
                                    width: `${sActivePct}%`,
                                    backgroundColor: ACCENT[site.slug],
                          ...
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <span className="text-[11px] text-[var(--pa-label)]">
                              {days === undefined ? (
                                ...
                                <>
                                  <span className="font-semibold tabular-nums text-[var(--pa-red)]">
                                    {lastErrorDay.count}
                                  </span>{" "}
...
function Sparkline({
  ...
  return (
    <div className="mt-2.5 flex flex-1 flex-col">
      <div className="flex min-h-7 flex-1 items-end gap-[3px]">
        {values.map((v, i) => (
          <span
            ...
            className={cn(
              "flex-1 rounded-[2px]",
              v > 0 ? "bg-[var(--pa-red)]" : "bg-[var(--pa-line)]",
            )}
            style={{
      ...
      <div className="mt-1.5 grid grid-cols-3 items-baseline text-[10px] uppercase tracking-wider text-[var(--pa-label)]">
...
function barClass(p: number): string {
  ...
  if (p < 30) return "bg-[var(--pa-red)]";
  if (p < 70) return "bg-[var(--pa-label)]";
  return "bg-[var(--pa-green)]";
}
