...
// =============================================================
// Dropdown Config Alpha5's client: **Triage queue**.
// =============================================================
...
import { RelatedCount } from "@/components/automations/related-count";
import {
...
export function DropdownConfigAlpha5Client({
  initialChoices,
  initialWebhooks,
}: {
  ...
  /** ⭐ THE SELECTION, which is what makes this a queue rather than a table.
   *  Cleared on tab switch: a GHL Tag id means nothing in GHL Forms. */
  ...
  /** Set one status on every selected row.
   *
   *  ⚠⚠ IT LOOPS THE ORDINARY PER-ROW ENDPOINT, four at a time. There is no
   *  bulk API, and **inventing one for a bench would be the tail wagging the
   *  dog**: the point here is the LAYOUT, and a loop proves the interaction
   *  without a migration. If this design wins, a real `PATCH /bulk` is the
   *  first thing to build, because 428 rows at four requests a batch is a lot
   *  of round trips for one decision.
   *  📌 THE BATCH SIZE IS DELIBERATE. Firing 400 requests at once is how you
   *  take the server down; see [[db-pool-max-10-fanout]] for the read-side
   *  version of that lesson.
   *  📌 IT SENDS `value` AND `notes` BACK UNCHANGED because the endpoint takes
   *  a whole record, not a patch of one field. Dropping them would blank the
   *  notes on every row you triaged. */
  async function bulkSetStatus(
        ...
        slice.map(async (row) => {
          try {
            const res = await fetch(
              `/api/automations/dropdown-choices/${row.id}`,
              {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    setBulkBusy(false);
    if (failed > 0) toast.error(`${failed} of ${rows.length} failed to save`);
    else toast.success(`${rows.length} set to ${status}`);
    router.refresh();
    // `table` is unused by the write but kept in the signature so a caller
    // cannot accidentally run this against the webhooks table, which has a
    // different endpoint and no status at all.
    void table;
  }
    ...
    <TooltipProvider delay={TOOLTIP_DELAY_MS}>
      <div className="space-y-6">
        ...
        <ChoiceTableSection
          ...
          bulkBusy={bulkBusy}
          onBulkStatus={(rows, status) =>
            bulkSetStatus(activeDescriptor, rows, status)
          }
        />
...
function ChoiceTableSectionTable({
  table,
  items,
  editMode,
  query,
            ...
            <table className="w-full min-w-[800px] table-fixed text-sm">
              ...
              <thead className="bg-zinc-50 text-xs uppercase tracking-wide text-zinc-500 [&_th]:sticky [&_th]:top-0 [&_th]:z-10 [&_th]:bg-zinc-50">
                <tr>
                  ...
                  {table.hasRelationships && (
                    <th className="w-[240px] px-3 py-2 text-center">
                      Relationships
                    </th>
                  )}
                  {table.hasNotes && (
                      ...
                      <td className="w-[240px] min-w-[240px] max-w-[240px] px-3 py-2 text-left align-top">
                        ...
                        item.relatedAutomations.length > 0 ? (
                          <div
                            ...
                            style={{
                              maxHeight: (notesClamp[item.id] ?? 2) * 16,
                            }}
                          >
                            ...
                            <TooltipTrigger
                              render={
                                <button
                                  ...
                                  style={{
                                    WebkitLineClamp: notesClamp[item.id] ?? 2,
                                  }}
                                >
                                  {item.notes}
                                </button>
...
/* ⭐⭐ A QUEUE FOR THE TABS THAT ARE ONE, A TABLE FOR THE REST.
 * GHL Tags (428) and GHL Forms (45) are not curated lists, they are SYNCED
 * ones: rows arrive from GoHighLevel and somebody has to decide, per row,
 * whether to keep it. That is a queue, and the live page renders it as a
 * spreadsheet where the only bulk operation is your patience.
 * 📌 THE FOUR COLOUR TABS AND WEBHOOK LINKS KEEP THE TABLE, because nobody
 * triages five evaluation options. */
interface QueueProps {
  ...
  bulkBusy: boolean;
  onBulkStatus: (rows: Item[], status: string) => void;
}
...
function TriageQueue({
  ...
  }, [items, query, table.statusOptions]);

  /** Rows bucketed by status, in the status set's own order. ⚠️ EMPTY BUCKETS
   *  STILL RENDER THEIR HEADING with a 0, unlike the search results on Alpha6:
   *  here "nothing is left To Remove" is the good news you came for, and a
   *  missing heading reads as a missing feature. */
  const groups = useMemo(() => {
    const options = table.statusOptions ?? [];
    return options.map((opt) => ({
      status: opt.value,
      rows: filtered.filter((i) => (i.status ?? "") === opt.value),
    }));
  }, [filtered, table.statusOptions]);

  const selectedRows = filtered.filter((i) => selectedIds.has(i.id));
  const bulkTargets = selectableStatusOptions(table.statusOptions ?? []);
  const { ref: scrollRef, style: scrollStyle } = useFitViewportHeight();
  ...
  function toggleGroup(rows: Item[], on: boolean) {
    const next = new Set(selectedIds);
    for (const r of rows) {
      if (on) next.add(r.id);
  ...
  return (
    <section className="space-y-3">
      ...
      {/* ⭐ THE BULK BAR IS THE WHOLE POINT. It appears only with a selection,
          so the page is quiet until you are actually triaging. */}
      {selectedRows.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 rounded-lg border border-zinc-900 bg-zinc-900 px-3 py-2 text-sm text-white">
          <span className="font-semibold tabular-nums">
            {selectedRows.length}
          </span>
          selected
          <span className="mx-1 text-zinc-500">|</span>
          <span className="text-xs text-zinc-300">Set status to</span>
          {bulkTargets.map((opt) => (
            <button
              key={opt.value}
              type="button"
              disabled={bulkBusy}
              onClick={() => onBulkStatus(selectedRows, opt.value)}
              className="rounded-md bg-white/10 px-2 py-1 text-xs font-medium transition hover:bg-white/20 disabled:opacity-50"
            >
              {opt.value}
            </button>
          ))}
          <button
            ...
            className="ml-auto text-xs text-zinc-300 underline-offset-2 hover:text-white hover:underline"
          >
            Clear
          </button>
        </div>
        ...
        >
          {filtered.length === 0 ? (
            ...
            groups.map(({ status, rows }) => {
              ...
              return (
                <div key={status}>
                  {/* Sticky group heading: which bucket you are in stays
                      visible through a 428-row scroll. */}
                  <div className="sticky top-0 z-10 flex items-center gap-2 border-b bg-zinc-50 px-3 py-1.5">
                    <input
                      type="checkbox"
                      checked={allOn}
                    ...
                    <StatusBadge
                      ...
                      options={table.statusOptions ?? []}
                    />
                    <span className="text-[10px] uppercase tracking-wider tabular-nums text-zinc-500">
                      {rows.length}
                    </span>
                  </div>
                  {rows.length === 0 ? (
                      ...
                      {rows.map((item) => {
                        ...
                        return (
                          <li
                            key={item.id}
                            className={cn(
                              "flex items-center gap-2 border-b px-3 py-1 text-sm",
                          ...
                          >
                            ...
                            />
                            {/* 📌 DENSE ROWS: py-1, not py-2. At 428 rows every
                                4px of row height is another screenful of
                                scrolling, and nothing here needs the air. */}
                            <button
                              type="button"
                              onClick={() => onEdit(item)}
                            ...
                            {table.hasRelationships && (
                              <button
                                ...
                                className="shrink-0 text-xs tabular-nums text-zinc-500 hover:text-zinc-900"
                              >
                            ...
                            {notes ? (
                              <button
                                type="button"
                                onClick={() => onShowNotes(notes)}
                                title={notes}
                                className="max-w-[45%] shrink-0 truncate text-xs text-zinc-500 hover:text-zinc-900"
                              >
                                {notes}
                              </button>
                            ) : null}
      ...
      {/* 🛑 NOT BUILT, AND SAID SO RATHER THAN IMPLIED: keyboard triage (j/k to
          move, space to select, 1-4 to set a status) is the obvious next step
          for a queue and is NOT here. It needs focus management and a roving
          tabindex, which is real work and proves nothing about the layout.
          📌 Edit mode is ignored by this queue: a row's NAME is a button that
          always opens the editor, because in a triage list clicking the thing
          you are triaging should never be inert. */}
      <p className="text-xs text-zinc-500">
        {editMode
          ? "Edit mode is on, though this queue does not need it: click a name to edit."
          : "Click a name to edit it, or select rows to set a status in bulk."}
      </p>
    </section>
  );
}
