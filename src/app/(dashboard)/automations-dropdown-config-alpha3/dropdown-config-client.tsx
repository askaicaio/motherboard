...
// =============================================================
// Dropdown Config Alpha3's client: **Swatch grid**.
// =============================================================
...
export function DropdownConfigAlpha3Client({
  initialChoices,
  initialWebhooks,
}: {
...
/* ⭐⭐ THE LAYOUT IS CHOSEN PER TAB, WHICH IS THE WHOLE IDEA OF THIS BENCH.
 * The live page renders one table for three different data shapes. Here the
 * four COLOUR sets (Author, Automation Tags, Trigger Event, Evaluation) render
 * as the badges they actually produce, and the rest keep the table.
 *
 * 📌 WHY ONLY THE COLOUR SETS: a swatch grid shows you the PALETTE at a glance,
 * which is the only question those tabs answer - "what do these look like, and
 * do any two clash?" The live table answers it with two cells of colour NAMES
 * ("Pale Brown", "Dark Brown") beside a pill, which is the same information
 * spelled out instead of shown.
 * 🛑 AND WHY NOT THE OTHERS: a grid of 428 GHL tags is unreadable, and those
 * rows carry a status and a notes column a swatch cannot hold. **A layout that
 * suits 15 items does not scale to 428**, and pretending otherwise is how the
 * live page ended up with one table for everything. */
function ChoiceTableSection(props: {
  table: TableDescriptor;
  items: Item[];
  editMode: boolean;
  query: string;
  ...
  if (props.table.hasColor) return <SwatchGrid {...props} />;
  return <ChoiceTableSectionTable {...props} />;
}

/** The colour sets, rendered as the badges they produce. */
function SwatchGrid({
  table,
  items,
  editMode,
  ...
  const filtered = useMemo(() => {
    ...
    const matched = q
      ? items.filter((i) => i.value.toLowerCase().includes(q))
      : items;
    return sortSpecialFirst(table.id, matched);
  }, [items, query, table.id]);
      ...
      <Card>
        <CardContent className="p-4">
          {filtered.length === 0 ? (
            ...
            <>
              {/* ⚠️ `items-start` KEEPS THE BADGES THEIR OWN HEIGHT. Without it a
                  flex row stretches every child to the tallest, and a two-line
                  badge would make its whole row of one-line badges tall and
                  hollow. */}
                ...
                {filtered.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      title={item.notes?.trim() || item.value}
                      className={cn(
                    ...
                    >
                      {/* The real pill, at the size it renders in the app. */}
                      <ColorPill
                        value={item.value}
                        badgeColor={item.badgeColor}
                        textColor={item.textColor}
              ...
              {/* 📌 THE NOTES DO NOT VANISH, they move to a hover and a click.
                  On these tabs Notes is a sentence on a handful of rows, not a
                  column worth ~900px of every screen. Any option carrying one
                  is clickable; the rest are inert unless Edit mode is on. */}
              <p className="mt-4 border-t pt-3 text-xs text-zinc-500">
                {editMode
                  ? "Edit mode is on: click any option to edit it."
                  : "Click an option with notes to read them, or turn on Edit mode to change one."}
              </p>
            </>
          )}
        </CardContent>
