// =============================================================
// Automations "Beta1" bench, route /automations-beta1
// =============================================================
// ⭐ SEEDED FROM THE LIVE HUB ON 2026-09-10, at the user's instruction: "For
// Beta1, Create the page mirroring the current official page." It began as a
// BYTE-IDENTICAL copy of `automations/page.tsx`, and the only differences are
// the three a bench must have: this header, the exported function's name, and
// the black "Beta1" badge.
//
// ⚠️⚠️ SO IT IS NOT A NEW DESIGN. It is a fresh, safe starting point for the
// next round of experiments on the live hub's CURRENT design, in the same way
// the old Beta bench started as a copy and was then reshaped. **Expect it to
// diverge from `/automations` the moment work begins; that is the point.**
//
// 📌 WHY BETA1 AND WHY NOW. The name was deliberately freed on 2026-09-10 by
// renumbering the other two benches up ("We have plans for another Beta1 page
// later"), and `/automations-beta1` was reserved for it. The rest of the family
// is Beta2 (Alpha3's master-and-detail, the most worked-on bench) and Beta3
// (seeded from Alpha2).
//
// ⚠️⚠️ THE VERSION-INDEPENDENCE RULE APPLIES HERE AS IT DOES TO EVERY BENCH:
// **DO NOT extract shared components between this file and
// `automations/page.tsx`**, however duplicated they look right now. They are at
// their most duplicated on the day of the copy, and that is exactly when
// factoring them out is most tempting and most damaging: the whole safety
// property of this scheme is that a bench experiment cannot break the live hub.
// Shared components under `src/components/automations/` are fine to IMPORT; it
// is this page's own layout that must stay its own.
//
// ⚠️ `/automations` IS FROZEN and was NOT touched to make this. The copy went
// one way.
//
// ⚠️ THE READS ARE SEVEN SEQUENTIAL `await`s, COPIED AS-IS AND DELIBERATELY NOT
// "FIXED". The live hub pays seven round trips; Beta2 runs its reads in two
// parallel waves instead (see its own note, and [[db-pool-max-10-fanout]] in
// memory for why the waves are capped). **Mirroring means mirroring, including
// the slow parts.** If this bench is ever the one being optimised, the wave
// pattern is there to copy, and the pool cap is SIX reads per wave.
//
// ⚠️ NO SELF-LINKS EXIST IN THIS PAGE, which is why the copy needed no href
// rewriting. Every link points either at a SHARED sub-page
// (/automations/all, /automations/dropdown-config,
// /automations/feature-integration) or at a per-website page
// (/automations/<slug>, /automations/<slug>/errors). Those are correct for a
// bench to use as-is; they are not the live hub's main page.
//
// ⬇️⬇️ EVERYTHING BELOW THIS LINE IS THE LIVE HUB'S OWN HEADER, INHERITED
// VERBATIM WITH THE COPY. It describes /automations and its history, not this
// bench. Read "this page" in it as "the live hub". It was kept rather than
// deleted because it explains why the design is shaped the way it is, which is
// exactly what someone experimenting on that design needs.
// =============================================================
...
//
// ✅ THE DUPLICATES ARE RESOLVED, AND THE RULE IS ONE COPY EACH, NOT A FIXED
// HOME. The card briefly said three things twice while the user compared both
// treatments on production; they then chose the STRIP's copy of every one
// ("alright, remove those two"). Two of the three have since MOVED AGAIN, and
// each still appears exactly once:
//   - auto-refresh state  -> still in the footer strip. (The
//                            "Auto-refresh: ✓/✗" row is gone, and with it the
...
export default async function AutomationsBeta1Page() {
  await requireAuth();
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                {/* ⚠️ THE BADGE IS THE ONLY VISIBLE DIFFERENCE FROM THE LIVE
                    HUB TODAY, and on the live hub this is a comment saying why
                    there is none there ("an unbadged title is the tell"). Here
                    the pill is the tell in the other direction.
                    ⚠️ IT IS THE *SHORT* NAME, matching every other bench: the
                    directory tile on the Feature Integration page reads "Main
                    Page Beta1", this pill reads just "Beta1", because it sits
                    beside an <h1> already saying "Automations". **The two
                    strings are meant to differ; only the version token has to
                    match.** Full reasoning in `@/lib/automations/versions`. */}
              ...
              return (
                ...
                >
                  ...
                  <CardContent className="flex h-full flex-col gap-3 p-5 pb-3">
                    ...
                    the View list button to be top aligned (2026-08-31), so the
                    button pins to the top of this row instead of centring
                    against the 44px logo tile. **Still wanted now that a PAIR
                    sits there** (2026-09-04): both buttons pin to the top and
                    read as a header action, rather than floating level with the
                    description. The INNER wrapper below keeps its own
