...
const WEBHOOK_PATH = "/webhook/provision-clickup";

export const clickupProvider: ProvisioningProvider = {
  toolKey: "clickup",
  displayName: "ClickUp",
  icon: "CheckSquare",
  supportsRetry: true,
  ...
  parseN8nCallback(callbackData: unknown): ProvisioningResult {
    ...
    if (data.status === "success") {
      return {
        success: true,
        data: {
          clickupUserId: data.clickupUserId,
          teams: data.teams,
        },
    ...
    return {
      success: false,
      errorMessage: data.error ?? "ClickUp provisioning failed",
