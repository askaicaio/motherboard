...
// Affiliate portal notification bell — the affiliate-facing counterpart of the
// staff NotificationBell. Same UX (red unread badge, Inbox/Archived tabs,
// per-item read/unread/archive, 60s badge poll) but talks to /api/portal/
// notifications* (partner-cookie auth) and opens downward from the header.
  ...
  const load = useCallback(async (which: "inbox" | "archived") => {
    ...
    try {
      const res = await fetch(
        `/api/portal/notifications${which === "archived" ? "?view=archived" : ""}`,
        { cache: "no-store" },
      );
      if (!res.ok) return;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        ...
        className="relative inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-200"
      >
            ...
            <ul className="divide-y divide-slate-100">
              {rows.map((r) => (
                <li
                  key={r.id}
                  className={cn(
                    "group/notif relative flex gap-2.5 px-3 py-2.5 transition hover:bg-slate-50",
